      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="glass-card p-8 mb-8 border-xenova-blue/30 shadow-neon">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <button
              onClick={logout}
              className="btn-neon text-xs py-2 opacity-60 hover:opacity-100"
            >
              Logout
            </button>
          </div>
        ...
        {bestRank && (
          <div
            className={`glass-card p-5 mb-8 text-center ${RANK_CONFIG[bestRank].border} shadow-neon`}
          >
            <p className="font-orbitron text-xs tracking-widest text-gray-500 mb-2">
              HIGHEST RANK
            </p>
            <div
              className={`inline-flex items-center gap-3 px-6 py-3 rounded-lg ${RANK_CONFIG[bestRank].bg} ${RANK_CONFIG[bestRank].border} border`}
            >
              <span className={`text-2xl ${RANK_CONFIG[bestRank].color}`}>
                {RANK_CONFIG[bestRank].icon}
              </span>
              <span
                className={`font-orbitron text-xl font-bold ${RANK_CONFIG[bestRank].color}`}
              >
                {bestRank}
              </span>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div key={label} className="glass-card p-4 text-center">
              ...
              <p className="font-orbitron text-xs text-gray-500 tracking-wider mt-1">
                {label}
              </p>
            </div>
          ))}
        ...
        <div className="glass-card overflow-hidden">
          <div className="px-6 py-4 border-b border-xenova-blue/10 flex items-center justify-between">
            ...
            <Link to="/quiz" className="btn-neon text-xs py-1.5">
              Take Quiz →
            </Link>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-12 text-center">
              <p className="text-4xl text-xenova-blue/10 font-orbitron mb-3">
                ◇
              </p>
              <p className="font-orbitron text-gray-600 tracking-widest text-sm mb-4">
                NO QUIZ ATTEMPTS YET
              </p>
              <Link to="/quiz" className="btn-neon text-sm">
            ...
            <div className="divide-y divide-xenova-blue/5">
              {[...scores].reverse().map((s, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="px-6 py-4 flex items-center gap-4 hover:bg-xenova-blue/5 transition-colors"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className={`font-orbitron text-xs ${cfg.color}`}>
                          {cfg.icon} {rank}
                        </span>
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="font-orbitron text-lg font-bold neon-text">
                        {s.percentage}%
                      </p>
