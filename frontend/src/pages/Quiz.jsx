...
// Direct axios — no dependency on api.js exports
...
export default function Quiz() {
  ...
  if (loading) return (
    <div className="min-h-screen star-bg flex items-center justify-center">
      <p className="font-orbitron text-xenova-blue animate-pulse tracking-widest">LOADING QUIZ...</p>
    </div>
  );
      ...
      <div className="min-h-screen star-bg pt-24 pb-16 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <div className="glass-card p-10 border-xenova-blue/40 shadow-neon">
            ...
            <p className="font-orbitron text-xs tracking-widest text-xenova-blue/60 mb-2">QUIZ COMPLETE</p>
            <h2 className={`font-orbitron text-3xl font-bold mb-4 ${RANK_COLORS[rank]}`}>{rank}</h2>

            {/* Score saved badge */}
            {scoreSaved && (
              <div className="inline-flex items-center gap-2 mb-4 px-4 py-1.5 rounded-full border border-green-400/30 bg-green-400/10">
                <span className="text-green-400 text-xs font-orbitron tracking-wider">✓ SCORE SAVED TO YOUR PROFILE</span>
              </div>
            )}
            {!isLoggedIn && (
              <div className="inline-flex items-center gap-2 mb-4 px-4 py-1.5 rounded-full border border-xenova-blue/20 bg-xenova-blue/5">
                <span className="text-gray-500 text-xs font-orbitron">
                  <Link to="/login" className="text-xenova-blue hover:underline">Login</Link> to save your score
                </span>
              </div>
            )}
            ...
            <div className="relative w-36 h-36 mx-auto my-8">
              <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
                <circle cx="18" cy="18" r="15.9" fill="none" stroke="rgba(0,243,255,0.1)" strokeWidth="2.5"/>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="font-orbitron text-3xl font-bold neon-text">{result.percentage}%</span>
            ...
            <div className="space-y-2 mb-8 text-left">
              {result.results.map((r, i) => (
                <div key={i} className={`flex items-start gap-3 p-3 rounded border text-sm ${
                  r.isCorrect ? 'border-green-400/20 bg-green-400/5' : 'border-red-400/20 bg-red-400/5'
                }`}>
                  <span className={`font-orbitron text-xs mt-0.5 flex-shrink-0 ${r.isCorrect ? 'text-green-400' : 'text-red-400'}`}>
                    {r.isCorrect ? '✓' : '✗'} Q{i + 1}
                  </span>
                  <p className={`text-xs ${r.isCorrect ? 'text-gray-300' : 'text-gray-500'}`}>{r.question}</p>
                </div>
              ))}
            </div>

            <div className="flex flex-wrap gap-3 justify-center">
              <button onClick={handleRetry} className="btn-neon">Try Again</button>
              {isLoggedIn && (
                <Link to="/profile" className="btn-purple">View My Scores</Link>
              )}
            </div>
    ...
    <div className="min-h-screen star-bg pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          <p className="font-orbitron text-xs tracking-widest text-xenova-blue/60 mb-3">KNOWLEDGE ASSESSMENT</p>
        ...
        <div className="glass-card p-4 mb-6">
          ...
          <div className="h-1.5 bg-xenova-blue/10 rounded-full overflow-hidden">
            <div className="h-full bg-xenova-blue rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
          </div>
          ...
          <div className="glass-card p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`text-xs font-orbitron px-2 py-0.5 rounded border ${
                questions[current].difficulty === 'Hard'   ? 'text-red-400 border-red-400/30 bg-red-400/10' :
                questions[current].difficulty === 'Medium' ? 'text-yellow-400 border-yellow-400/30 bg-yellow-400/10' :
                                                             'text-green-400 border-green-400/30 bg-green-400/10'
              }`}>
                {questions[current].difficulty}
              </span>
            </div>
            <p className="text-white text-base leading-relaxed mb-6">{questions[current].question}</p>
            <div className="space-y-2.5">
              {questions[current].options.map((opt, i) => {
                const selected = answers[questions[current]._id] === i;
                return (
                  <button key={i} onClick={() => handleSelect(questions[current]._id, i)}
        ...
        <div className="flex justify-between">
          ...
          {current < questions.length - 1 ? (
            <button onClick={() => setCurrent(c => c + 1)} className="btn-neon">Next →</button>
          ) : (
